...
+
Weitere Informationen https://["Gehen Sie zur Astra Control Center-Dokumentation"^].
...
+
https://["Erfahren Sie, wie Sie das Lifecycle Management von Applikationsdaten mit der REST-API automatisieren"^].
...
Der Astra Control Service unterstützt jetzt das https://["_CVS_ Diensttyp"^] Mit Cloud Volumes Service für Google Cloud. Dies unterstützt zusätzlich bereits den Servicetyp _CVS-Performance_. Zur Erinnerung: Astra Control Service nutzt Cloud Volumes Service für Google Cloud als Storage-Backend für Ihre persistenten Volumes.

Dank dieser Verbesserung kann der Astra Control Service jetzt Applikationsdaten für Kubernetes-Cluster managen, die in _any_ ausgeführt werden https://["Google Cloud-Region, in der Cloud Volumes Service unterstützt wird"^].
